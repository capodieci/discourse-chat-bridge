...
- Consequences: one rebuild is needed, and it is the same rebuild that installs the plugin, so the cost is paid once. After that, adding or removing an embedding site is an admin UI change with no downtime. Note that this configuration sends `Access-Control-Allow-Credentials: true` to listed origins, so a cross site scripting hole on a listed site becomes a route to a visitor's forum account. Keep the list short and under our control.
